...
def add_favorite_location(request):
    if request.method == 'POST':
        user = request.user
        location_id = request.POST.get('location_id')  # Assuming the select input's name is 'location'
        location = get_object_or_404(Location, pk=location_id)
        # Check if the location is already a favorite
        existing_favorite = FavoriteLocation.objects.filter(user=user, location=location).exists()
